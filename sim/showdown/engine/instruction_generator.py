...
def get_instructions_from_switch(mutator, attacker, switch_pokemon_name, instructions):
    ...
    if switch_pkmn.item != 'heavydutyboots':
        ...
        if attacking_side.side_conditions[
            constants.STICKY_WEB] == 1 and switch_pkmn.is_grounded() and switch_pkmn.ability not in constants.IMMUNE_TO_STAT_LOWERING_ABILITIES:
...
def get_instructions_from_statuses_that_freeze_the_state(mutator, attacker, defender, move, opponent_move, instruction):
    ...
    if constants.PARALYZED == attacker_side.active.status:
        ...
        if move[constants.ID] not in constants.THAW_IF_USES and opponent_move.get(constants.ID) not in constants.THAW_IF_HIT_BY and opponent_move.get(
                constants.TYPE) != 'fire':
