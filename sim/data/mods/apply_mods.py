...
from sim.showdown.engine import damage_calculator
...
def apply_move_mods(gen_number):
    ...
    for gen_number in reversed(range(gen_number, CURRENT_GEN)):
        path = base / f"gen{gen_number}_move_mods.json"
        with path.open() as f:
...
def apply_pokedex_mods(gen_number):
    ...
    for gen_number in reversed(range(gen_number, CURRENT_GEN)):
        path = base / f"gen{gen_number}_pokedex_mods.json"
        with path.open() as f:
...
def set_random_battle_sets(gen_number):
    ...
    path = base / f"/random_battle_sets_gen{gen_number}.json"
    with path.open() as f:
