...
def get_effective_priority(side, move, field):
    ...
    if side.active.ability == 'prankster' and move[constants.CATEGORY] == constants.STATUS:
        priority += 1
    elif side.active.ability == 'galewings' and (side.active.hp == side.active.maxhp) and ('flying' in move[
        constants.TYPE]):
        priority += 1
...
def update_attacking_move(attacking_side, attacking_pokemon, defending_pokemon, attacking_move, defending_move, first_move, weather, terrain):
    ...
    if (
        ...
        if constants.BANEFUL_BUNKER in defending_pokemon.volatile_status and constants.CONTACT in attacking_move[
            constants.FLAGS]:
            ...
            if constants.CRASH in attacking_move:
                ...
                attacking_move[constants.HEAL] = [-1 * attacking_move[constants.CRASH][0], attacking_move[
                    constants.CRASH][1]]
        elif constants.SPIKY_SHIELD in defending_pokemon.volatile_status and constants.CONTACT in attacking_move[
            constants.FLAGS]:
            attacking_move[constants.ACCURACY] = True
            attacking_move[constants.CATEGORY] = constants.STATUS
        ...
        elif constants.SILK_TRAP in defending_pokemon.volatile_status and constants.CONTACT in attacking_move[
            constants.FLAGS]:
            attacking_move[constants.ACCURACY] = True
            attacking_move[constants.CATEGORY] = constants.STATUS
...
def get_state_instructions_from_move(mutator,
    ...
    if constants.SWITCH_STRING in attacking_move:
        return [instruction_generator.get_instructions_from_switch(mutator, attacker, attacking_move[
            constants.SWITCH_STRING], instructions)]
    ...
    for instruction_set in all_instructions:
        temp_instructions += instruction_generator.get_instructions_from_move_special_effect(mutator, attacker, attacking_pokemon, defending_pokemon, attacking_move[
            constants.ID], instruction_set)
        ...
        for i in all_instructions:
            ...
            if best_switch is not None:
                temp_instructions.append(
                    instruction_generator.get_instructions_from_switch(mutator, attacker, best_switch, i))
            else:
