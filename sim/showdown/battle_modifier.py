...
from sim.showdown.battle import StatRange
...
from sim.showdown.engine.find_state_instructions import get_effective_speed
from sim.showdown.engine.damage_calculator import calculate_damage
...
def move(battle, split_msg):
    ...
    if category in constants.DAMAGING_CATEGORIES and not any([normalize_name(a) in ['sheerforce', 'magicguard'] for a in pokedex[pkmn.name][
...
def upkeep(battle, _):
    ...
    if battle.opponent.side_conditions[constants.PROTECT] > 0:
        ...
        logger.debug("Setting protect to {} for the opponent".format(battle.opponent.side_conditions[
                                                                         constants.PROTECT]))
...
def check_choicescarf(battle, msg_lines):
    ...
    moves = [get_move_information(m) for m in msg_lines if m.startswith('|move|')]
    if len(moves) != 2 or moves[0][0].startswith(battle.user.name) or moves[0][1][constants.PRIORITY] != moves[1][1][
        constants.PRIORITY]:
        return
